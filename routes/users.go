...
func login(context *gin.Context) {
	...
	err := context.ShouldBindJSON(&user)
	if err != nil {
		context.JSON(http.StatusBadRequest, gin.H{"message": "Failed to parse login request data"})
		return
	}
	...
	if err != nil {
		context.JSON(http.StatusInternalServerError, gin.H{"message": "Failed to generate JWT token"})
	}
